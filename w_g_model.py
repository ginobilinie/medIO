...
class MR2CT(object):
    def __init__(self, sess, batch_size=10, height_MR=64,width_MR=64, height_CT=48,
                 ...
                 lam_lp=1, lam_gdl=1, lam_adv=1, alpha=2, exclude_id=0):
        ...
        self.data_generator = Generator_2D_center_slices(path_patients_h5,self.batch_size,self.exclude_id)
        self.build_model()

    def build_model(self):
	    with tf.device('/gpu:0'):
			self.inputMR=tf.placeholder(tf.float32, shape=[None, self.height_MR, self.width_MR, 5])#5 chans input
			self.inputMR_center=tf.placeholder(tf.float32, shape=[None, self.height_CT, self.width_CT, 1])#I'd like to maintain the center part of inputMR 
			...
			self.G, self.layer = self.res_generator(self.inputMR,self.inputMR_center, batch_size_tf)#by dong, add res unit
			print 'G shape ',self.G.get_shape
			self.D, self.D_logits = self.w_discriminator(self.CT_GT)#real CT data, return: after sigmoid, before sigmoid
			self.D_, self.D_logits_ = self.w_discriminator(self.G, reuse=True)#fake generated CT data
			#self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits, tf.ones_like(self.D)))
			#self.d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits_, tf.zeros_like(self.D_)))                 
			self.d_loss_real = tf.reduce_mean(self.D_logits)#real for wgan
			self.d_loss_fake = tf.reduce_mean(self.D_logits_)#fake for wgan
...
############by dong, add res unit from the input to the output###############################
    def res_generator(self,inputMR,inputMR_center, batch_size_tf):
               
        ######## FCN for the 32x32x32 to 24x24x24 ###################################
        print 'input shape, ',inputMR.get_shape()
        conv1_a = conv_op_bn(inputMR, name="g_conv1_a", kh=7, kw=7, n_out=128, dh=1, dw=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)#64->58
        conv2_a = conv_op_bn(conv1_a, name="g_conv2_a", kh=5, kw=5, n_out=128, dh=1, dw=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)#54
        conv3_a = conv_op_bn(conv2_a, name="g_conv3_a", kh=3, kw=3, n_out=256, dh=1, dw=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)#52
        conv4_a = conv_op_bn(conv3_a, name="g_conv4_a", kh=3, kw=3, n_out=256, dh=1, dw=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)#50
        conv5_a = conv_op_bn(conv4_a, name="g_conv5_a", kh=3, kw=3, n_out=128, dh=1, dw=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)#48
        ...
        print 'inputMR_center shape, ', inputMR_center.get_shape()
        #tmpMR=tf.image.central_crop(tmpMR,0.75)#64->48, or we can add one convolution
        #print 'tmpMR shape, ',tmpMR.get_shape() 
        conv9_a=conv9_a+inputMR_center
        print 'conv9a shape, ',conv9_a.get_shape()
        #self.MR_16_downsampled=conv7_a#JUST FOR TEST
        return conv9_a,conv9_a
    ...
    def w_discriminator(self, inputCT, reuse=False):
        if reuse:
            tf.get_variable_scope().reuse_variables()
        print 'ct shape ',inputCT.get_shape()
        h0=conv_op_bn(inputCT, name="d_conv_dis_1_a", kh=5, kw=5, n_out=32, dh=1, dw=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)
    ...
    def train(self, config):
    	#path_test='/shenlab/lab_stor3/dongnie/3T7T-Data/testSubs/'
    	path_test='/home/dongnie/warehouse/3T7T-Data/testHistSubs/'
        ...
        for it in range(start,config.iterations):
            ...
            X,y,X_center=self.data_generator.next()
            ####the following codes are added by dong####            
            #print 'X shape, ',X.shape,' y shape, ',y.shape
            #X: batchsize, width, height, channel
            #y: batchsize, width, height, channel
#             Xw=X.shape[1]
#             yw=y.shape[1]
#             Xh=X.shape[2]
#             yh=y.shape[2]
...
#             marginH=(Xh-yh)/2
#             marginC=(Xc-yc)/2
#             #X_center=np.zeros(y.shape)
#             X_center=X[:,marginW:Xw-marginW,marginH:Xh-marginH,marginC:Xc-marginC]
            #print 'X_center shape, ',X_center.shape
            # Update D network
            _, loss_eval_D, = self.sess.run([self.d_optim, self.d_loss],
                        feed_dict={ self.inputMR: X,  self.inputMR_center: X_center, self.CT_GT:y, self.train_phase: True })
            ...
            if it%config.test_every==0 and it!=0:#==0:#test one subject                
                ...
                mr_test_itk=sitk.ReadImage(os.path.join(path_test,'S3to1_3t.nii.gz'))
                ct_test_itk=sitk.ReadImage(os.path.join(path_test,'S3to1_7t.nii.gz'))
                ...
                print mrnp.dtype
                print ctnp.dtype
                ct_estimated=self.test_1_subject(mrnp,ctnp,[64,64,5],[48,48,1],[5,5,1]) #will take long time, data has already been normalized 
                psnrval=psnr(ct_estimated,ctnp)
                print ct_estimated.dtype
                print ctnp.dtype
                print 'psnr= ',psnrval
                volout=sitk.GetImageFromArray(ct_estimated)
                sitk.WriteImage(volout,'7t_estimated_{}'.format(it)+'.nii.gz')
    ...
    def combined_loss_G(self,batch_size_tf):
        ...
        bceterm=-1*tf.reduce_mean(self.D_logits_)#for generator

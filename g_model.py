...
class MR2CT(object):
    ...
    def build_model(self):
        with tf.device('/gpu:0'):
			self.inputMR=tf.placeholder(tf.float32, shape=[None, self.height_MR, self.width_MR, 5])#5 chans in
    ...
    def train(self, config):
		...
		path_test='/shenlab/lab_stor3/dongnie/3T7T-Data/testHistSubs/'
		...
		for it in range(start,config.iterations):
           ...
           # Update D network
			...
			if it%config.test_every==0 and it!=0:#==0:#test one subject                
				...
				mr_test_itk=sitk.ReadImage(os.path.join(path_test,'S9to1_3t.nii.gz'))
				ct_test_itk=sitk.ReadImage(os.path.join(path_test,'S9to1_7t.nii.gz'))

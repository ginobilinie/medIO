...
Author: Dong Nie 
Note, this is specified for the prostate, which input is larger than output, and we input multi-scale input at the same time
    ...
    for i in range(0,row-dSeg[0]+1,step[0]):
        for j in range(0,col-dSeg[1]+1,step[1]):
            for k in range(0,leng-dSeg[2]+1,step[2]):
                ...
                mynet.blobs['dataMR16'].data[0,0,...]=volFAScale1
                mynet.blobs['dataMR8'].data[0,0,...]=volSegScale2
                mynet.forward()

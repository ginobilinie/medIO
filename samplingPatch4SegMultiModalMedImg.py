...
We further implement patch sampling using gradient map and magnitude map
...
'''
    Compute the gradient map and magnitude of nd image
'''
def computeGradMap(mat):
...
Here, we uniformly extract patches across the images
'''
...
#     trainSeg2D=trainSeg2D[0:cubicCnt,:,:,:]
    ...
    with h5py.File('./train32_%s.h5'%fileID,'w') as f:
        f['dataT1']=trainFA
        f['dataT2']=trainMR
        f['dataSeg']=trainSeg
...
#         f['dataSeg2D']=trainSeg2D
    ...
    with open('./train_iSeg_32_list.txt','a') as f:
        f.write(saveto+'train32_%s.h5\n'%fileID)
    return cubicCnt


'''
Here, we sample important patches across the images
'''
def samplePatches4OneSubject(matFA,matMR,matSeg,fileID,d,step,rate):
...
#     trainSeg2D=np.zeros([estNum,dSeg[0],dSeg[1],dSeg[2]],dtype=np.int8)
    ...
    ###compute gradient map to guide patch selection
#     gradFA=np.gradient(gradFA)
#     gradMR=np.gradient(matMROut)
#     volOut=sitk.GetImageFromArray(gradFA)
#     sitk.WriteImage(volOut,'preSub1_T1.nii.gz')
#     volOut=sitk.GetImageFromArray(gradMR)
#     sitk.WriteImage(volOut,'preSub1_T2.nii.gz')       
#     print 'we have save gradient map'    
    gradFA,magFA=computeGradMap(matFAOut)
    gradMR,magMR=computeGradMap(matMROut)    
    dsfactor = rate
    #actually, we can specify a bounding box along the 2nd and 3rd dimension, so we can make it easier 
    for i in range(0,row-dSeg[0]+1,step[0]):
        for j in range(0,col-dSeg[1]+1,step[1]):

...
mynet = caffe.Net(protopath+'prostate_deploy_v8_2.prototxt',protopath+'pro_fcn_v8_2_iter_100000.caffemodel',caffe.TEST)
        ...
        for j in range(0,col-d[1],step[1]):
            for k in range(0,leng-d[2],step[2]):
                ...
                if rate!=1 and i%resizeRate==0 and j%resizeRate==0 and k%resizeRate==0:
                    ...
                    temppremat = mynet.blobs['conv2c'].data[0] #Note you have add softmax layer in deploy prototxt
...
#         mrimg=(mrimg-mu)/(maxV-minV)
        ...
        sitk.WriteImage(volOut,'preSub%d_s16.nii'%myid)

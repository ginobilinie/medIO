...
mynet = caffe.Net(protopath+'prostate_deploy_v8_1.prototxt',protopath+'pro_fcn_v8_1_iter_100000.caffemodel',caffe.TEST)
...
#     sitk.WriteImage(myOut,'enlargeSub%s.nii.gz'%fileID)
        
    matOut=np.zeros([int(np.ceil(rate*matSeg.shape[0])),int(np.ceil(rate*matSeg.shape[1])),int(np.ceil(rate*matSeg.shape[2]))])
    used=np.zeros([int(np.ceil(rate*matSeg.shape[0])),int(np.ceil(rate*matSeg.shape[1])),int(np.ceil(rate*matSeg.shape[2]))])+eps
    ...
    for i in range(0,row-d[0],step[0]):
        for j in range(0,col-d[1],step[1]):
            for k in range(0,leng-d[2],step[2]):
                ...
                resizeRate=int(1/rate)
                if rate!=1 and i%resizeRate==0 and j%resizeRate==0 and k%resizeRate==0:
                    ...
                    temppremat = mynet.blobs['conv1c_bn'].data[0] #Note you have add softmax layer in deploy prototxt
                    #temppremat=np.zeros([volSeg.shape[0],volSeg.shape[1],volSeg.shape[2]])
                    #print 'shape tempremat ',temppremat.shape,'k and kend',k,k+int(rate*d[2])
                    
                    iRate=int(i/resizeRate)
                    jRate=int(j/resizeRate)
                    kRate=int(k/resizeRate)
                    matOut[iRate:iRate+int(rate*d[0]),jRate:jRate+int(rate*d[1]),kRate:kRate+int(rate*d[2])]+=np.squeeze(temppremat)
                    used[iRate:iRate+int(rate*d[0]),jRate:jRate+int(rate*d[1]),kRate:kRate+int(rate*d[2])]+=1;
                elif rate==1:
                    mynet.blobs['dataMR'].data[0,0,...]=volFA
                    mynet.forward()
                    #temppremat = mynet.blobs['softmax'].data[0].argmax(axis=0) #Note you have add softmax layer in deploy prototxt
                    ...
                    matOut[i:i+int(rate*d[0]),j:j+int(rate*d[1]),k:k+int(rate*d[2])]+=np.squeeze(temppremat)
                    used[i:i+int(rate*d[0]),j:j+int(rate*d[1]),k:k+int(rate*d[2])]+=1
                else:
                    continue
    matOut=matOut/used
...
#         mrimg=(mrimg-mu)/(maxV-minV)
        ...
        sitk.WriteImage(volOut,'preSub%d_s8.nii'%myid)
